...
# A technique-only match (agreeing ATT&CK technique but no shared event ID) is
# weaker still: it says a rule targets this abuse class, not that it fires on
# this edge's specific telemetry. Off by default so the Sigma tier stays event-ID
# grounded; the live-SIEM tiers (Elastic/Sentinel) opt in because their rules are
# largely behavioural queries that never name an EventID.
_TECHNIQUE_ONLY_PENALTY = 20
...
def compute_coverage(corpus, rules: list, allow_technique_only: bool = False) -> dict:
    ...
    share one event ID (e.g. everything under DS-Access 4662).

    ``allow_technique_only`` (off for Sigma, on for the live-SIEM tiers) also
    counts a rule whose ATT&CK technique agrees with the edge but which names no
    shared event ID - the dominant case for behavioural Elastic/Sentinel queries.
    Such matches carry a larger penalty and are labelled ``technique`` so the
    report never conflates them with event-grounded coverage.
    """
    coverage = {}
    for entry in corpus:
        et = entry["edge_type"]
        ...
        best = None
        for rule in rules:
            event_hit = bool(edge_ids & rule.event_ids)
            confirmed = _tech_match(rule.techniques, edge_tech)
            if event_hit:
                # If the rule is technique-scoped and the edge has a technique,
                # they must agree - otherwise the rule targets a different abuse
                # that merely shares the event ID.
            ...
            elif allow_technique_only and confirmed:
                match = "technique"
                floor = max(0, _LEVEL_FLOOR.get(rule.level, _LEVEL_FLOOR[_DEFAULT_LEVEL])
                             - _TECHNIQUE_ONLY_PENALTY)
            else:
                continue  # rule does not fire on this edge
...
def to_environment_profile(coverage: dict, name: str = "sigma-coverage",
                           source: str = "noisehound-sigma from deployed Sigma rules") -> dict:
    return {
        "_comment": "Generated by %s (v%s). adjustments raise edges covered by a "
                    "deployed detection toward a floor. Pass with --environment."
                    % (source, __version__),
        "name": name,
